...
def unique_values_by_column(df, threshold=0, plot_size=(12, 8)):
    ...
    ax = counts_df.plot(kind="bar", title="Number of Unique Values Per Column", figsize = plot_size)
...
def iqr_plot(df, factor, plot_size=(12, 8)):
    numerical_columns = df.select_dtypes(include=["number"]).columns
...
def scatter_plot(df, plot_size=(15, 10)):
    ...
    fig, axs = plt.subplots(total_rows, columns_per_row, figsize=plot_size)
